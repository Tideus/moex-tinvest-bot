...
Отчётность и безопасное Sandbox-исполнение:
[`REPORTS_AND_SANDBOX_RU.md`](REPORTS_AND_SANDBOX_RU.md).

## 1. Что бот умеет сейчас

Проект готов для `replay`, часового анализа и исполнения на sandbox-счёте:
он читает завершённые свечи MOEX,
проверяет свежесть, рассчитывает momentum/trend, применяет GeoRisk и риск-лимиты, создаёт
заявки-кандидаты, отправляет прошедшие risk-gate заявки в Sandbox API, пишет audit trail и
отправляет отчёты в Telegram через SQLite outbox.
...
Production-заявки заблокированы. Переключатель API `sandbox/prod` не равен разрешению исполнения:

| Понятие | Что меняет |
| --- | --- |
| `config/runtime.json`: `sandbox/prod` | Сервер T-Invest и пару token/account ID |
| `config/shadow.json`: `mode=shadow` | Создание проверенного плана и risk-gate |
| `runtime.json`: `sandbox_orders_enabled` | Отдельное разрешение исполнить план только в sandbox |
...
Отправляются три типа торговых отчётов:

- каждый час — цели, рассчитанные BUY/SELL и причины risk rejection;
- после sandbox-отправки — отдельный список заявок и статусов брокера;
- ежедневно в `23:20 Europe/Moscow` — broker equity, общий P&L и вклад каждой бумаги;
- в пятницу — недельный P&L, просадка, качество процесса и вывод о следующем шаге.

Дневная и недельная сводки дедуплицируются периодом. Общий результат считается по изменению
broker equity без пополнений/выводов; вклад бумаги — по её стоимости, исполненным операциям и
комиссии. Строка `Прочее/сверка счёта` показывает разницу, которую нельзя честно отнести к бумаге.
...
`Заявки для T‑Invest Sandbox` показывают разницу между текущим и целевым портфелем, ограниченную
...
Все суммы в утреннем сообщении до отправки являются планом. Отдельное сообщение
`SANDBOX · РЕАЛЬНОЕ ИСПОЛНЕНИЕ НА ТЕСТОВОМ СЧЁТЕ`
...
| Место | Содержание |
| --- | --- |
| `artifacts/hourly_shadow-*.json` | quality, GeoRisk, targets, заявки-кандидаты и отклонения |
...
- `targets`: желаемые веса Sandbox-портфеля после стратегии и GeoRisk.
- `orders`: прошедшие внутренний risk-gate заявки-кандидаты; фактическая отправка и broker status находятся в `sandbox-execution-*.json`.
...
Воспроизводимый тест текущей Strategy v2 на Ubuntu запускается командой
`sudo moex-botctl backtest`. Сверяйте зелёную кривую капитала с синей кривой price-only IMOEX в
`equity-curve.html`, но оценивайте не только относительное опережение: для перехода дальше
обязательны положительная абсолютная OOS-доходность после cost stress, приемлемая просадка и
положительный Sharpe. `market-data.json` сохраняет использованные реальные свечи, а
`promotion-assessment.json` — каждый пройденный и проваленный gate.

### Sandbox OOS: минимум 8 недель

Еженедельно считайте:

- долю успешных часовых циклов;
- число stale/blocked/ошибочных циклов;
- расхождение лимитной цены, broker fill и последующей доступной цены;
...
Команда `sudo moex-botctl sandbox-disable` немедленно запрещает новые заявки. Уже выставленные
заявки она не отменяет — их нужно сверить у брокера.

Недельные verdict трактуются так:

- `REVIEW_DATA` — устранить blocked/ошибки данных, стратегию пока не трогать;
- `COLLECT_MORE` — выборка мала, продолжить наблюдение;
- `REVIEW_RISK` — проверить размеры и лимиты, не переобучать сигнал автоматически;
- `OBSERVE` — отрицательная неделя, искать устойчивое OOS-ухудшение, а не реакцию на шум;
- `CONTINUE_OOS` — процесс стабилен, но одной недели недостаточно для изменения параметров.
...
4. Сверить `production orders: DISABLED` и ожидаемое состояние `sandbox orders`.
