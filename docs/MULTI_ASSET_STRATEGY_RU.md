...
## 9. Ближайший реализуемый инкремент

Следующая версия должна реализовать только Этап A:

1. получить read-only sandbox portfolio, positions и active orders;
2. получить available/blocked cash, equity, free margin и операции;
3. сохранить broker snapshot без секретов;
4. агрегировать текущие позиции по issuer/underlying/sector;
5. связать snapshot с часовым forecast artifact;
6. сформировать недельный forecast/deal/diversification scorecard;
7. оставить sandbox order submission и short выключенными.

Это закрывает главный пробел текущего бота и создаёт данные, без которых невозможно честно
оценить ни long/short модель, ни фьючерсы, ни опционы.
